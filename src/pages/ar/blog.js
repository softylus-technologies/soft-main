...
const Blog = ({ pageContext }) => {
    ...
    const fetchBlogs = async () => {
      try {
        const response = await axios.get(
          `https://strapi.softylus.com/api/blogs?populate=*&locale=ar`,
          {
            headers: {
  ...
  return (
    <AppWrapper pageContext={pageContext}>
       <Seo 
        title="مدونة Softylus | دليلك لأحدث اتجاهات وتطورات تطوير البرمجيات"
        description="ابقَ في مقدمة العصر الرقمي مع مدونة Softylus. اكتشف أحدث الاتجاهات، والرؤى المتخصصة، والدلائل الشاملة حول تطوير البرمجيات، DevOps، الحوسبة الطرفية، والمزيد. استمتع بتحليلات معمقة وكن على اطلاع دائم بالتطورات في عالم التكنولوجيا المتغير."
      />
      <Layout>
        {blogContent}
        <FooterCon
