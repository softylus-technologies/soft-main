  ...
  return (
    <div className="btn-sliider">
      <button
        ...
        aria-label={intl.formatMessage({ id: "projectDetail.previousSlide" })}
      >
        <img src='/Frame 1000003367.png' alt={intl.formatMessage({ id: "projectDetail.previousSlideAlt" })} />
      </button>
      <button
        onClick={() => swiper.slideNext()}
        className={isLastSlide ? 'btn-slide next disabled' : 'btn-slide next'}
        aria-label={intl.formatMessage({ id: "projectDetail.nextSlide" })}
      >
        <img src='/Frame 1000003367.png' alt={intl.formatMessage({ id: "projectDetail.nextSlideAlt" })} />
      </button>
    </div>
  );
};
...
const ProjectDetail = ({ params, location }) => {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      console.log("Fetching project with locale:", currentLocale);
      
      try {
        const url = `https://strapi.softylus.com/api/projects?filters[slug][$eq]=${slug}&locale=${currentLocale}&populate=images,TechnologyUsed`;
        ...
        if (response.data.data.length > 0) {
          ...
          if (response.data.data[0].attributes.locale !== currentLocale) {
            console.warn(`Requested locale ${currentLocale} not available, falling back to ${response.data.data[0].attributes.locale}`);
          }
        } else {
  ...
  if (!slug) {
    return <div><FormattedMessage id="projectDetail.noSlug" /></div>;
  }

  if (!project) {
    return <div><FormattedMessage id="projectDetail.loading" /></div>;
  }
  ...
  return (
    <Layout>
      <Seo
        title={intl.formatMessage({ id: "projectDetail.seoTitle" }, { title: title })}
        description={intl.formatMessage({ id: "projectDetail.seoDescription" }, { description: description })}
      />
          ...
          <div className="project-detail-conntent">
            <div className='project-detail-title'>
              ...
              <div className="mb-14 project-detail-someDetail-button">
                ...
                >
                  <FormattedMessage id="projectDetail.buildMeOne" />
                </Link>
              </div>
            </div>
